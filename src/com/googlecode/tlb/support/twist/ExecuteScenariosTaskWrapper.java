 ...
 * </ul>
 */
public class ExecuteScenariosTaskWrapper extends Task {
    ...
    public ExecuteScenariosTaskWrapper() {
        this(new Java());
    }

    ExecuteScenariosTaskWrapper(Java java) {
        this.java = java;
    }
    ...
    /**
     * @param errorproperty The name of a property to set in the event of an error.
     */
    ...
    /**
     * @param failureproperty The name of a property to set in the event of a failure (errors are considered failures as well).
     */
    ...
    private void haltOnErrorOrFailureIfNecessary(int errorCode) {
        switch (errorCode) {
            case ScenarioExecutorAntMainWrapper.ERROR_EXIT_CODE:
                if (haltonerror) {
                    throw new BuildException("Error(s) occured executing test(s) in '" + scenarioDir.getAbsolutePath() + "'", getLocation());
                }
                break;
            case ScenarioExecutorAntMainWrapper.FAILURE_AND_ERROR_EXIT_CODE:
                ...
                break;
            case ScenarioExecutorAntMainWrapper.FAILURE_EXIT_CODE:
                if (haltonfailure) {
